...
class SelesaiKegiatan extends StatefulWidget{
  const SelesaiKegiatan({Key? key}) :super (key: key);
      ...
      child: Scaffold(
        body: BlocBuilder<KegiatanCubit, KegiatanState>(
          builder: (context, state){
            ...
            return SelesaiKegiatanBody(model: context.select((KegiatanCubit bloc) => bloc.model));
          },
        ),
      ),
    );
...
class SelesaiKegiatanBody extends StatefulWidget{
  ...
  const SelesaiKegiatanBody({Key? key, required this.model}) : super (key: key);
